...
import { NotificationDomain, NotificationRepository } from '../../../../domain/notification.domain';
import * as admin from 'firebase-admin';

@Injectable()
export class NotificationRepositoryBaseService implements NotificationRepository {
    ...
    async getNotificationsByFollow(uid: string): Promise<NotificationDomain[]> {
        const notificationsRef = this.db.collection('notifications');
        const query = notificationsRef.where('uid', '==', uid).where('type', '==', 'follow');
        return query.get().then((snapshot) => {
            return snapshot.docs.map((doc) => doc.data() as NotificationDomain);
        });
    }
    async getNotificationsByLike(uid: string): Promise<NotificationDomain[]> {
        const notificationsRef = this.db.collection('notifications');
        const query = notificationsRef.where('uid', '==', uid).where('type', '==', 'like');
        return query.get().then((snapshot) => {
            return snapshot.docs.map((doc) => doc.data() as NotificationDomain);
        });
    }
    async getNotificationsByComment(uid: string): Promise<NotificationDomain[]> {
        const notificationsRef = this.db.collection('notifications');
        const query = notificationsRef.where('uid', '==', uid).where('type', '==', 'comment');
        return query.get().then((snapshot) => {
            return snapshot.docs.map((doc) => doc.data() as NotificationDomain);
        });
    }
